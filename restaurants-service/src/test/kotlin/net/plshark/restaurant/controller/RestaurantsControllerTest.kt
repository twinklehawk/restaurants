...
class RestaurantsControllerTest {
    ...
    fun `create should save the restaurant and the associated containers`() {
        ...
        every { restaurantContainersRepository.insert(321, match { it == 1L || it == 2L }) } returns Mono.just(1)
        val tc1 = TakeoutContainer(1, "1")
        val tc2 = TakeoutContainer(2, "2")
        every { restaurantContainersRepository.getContainersForRestaurant(321) } returns Flux.just(tc1, tc2)

        controller.create(create).test()
            .expectNext(restaurant.copy(containers = listOf(tc1, tc2)))
            .verifyComplete()
    }
    ...
    fun `findById should return the matching object`() {
        ...
        val tc = TakeoutContainer(123, "paper")
        every { restaurantContainersRepository.getContainersForRestaurant(321) } returns Flux.just(tc)

        controller.findById(321).test().expectNext(match.copy(containers = listOf(tc)))
            .verifyComplete()
    }
    ...
    fun `findById should throw a NotFoundException if no match is found`() {
        ...
        controller.findById(123).test().verifyError(NotFoundException::class)
    }
    ...
    fun `update should update the restaurant and the container types`() {
        ...
        controller.update(1, request).test().expectNext(request).verifyComplete()
    }
